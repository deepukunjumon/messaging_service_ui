...
import { updateApiClient } from "../../../services/apiclients.service";
...
export const EditClientModal = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await updateApiClient(client.id, {
        clientName,
        description,
      });

      showToast(res.data?.data?.message || "Client updated", "success");
      ...
      let errorMessage = backendError?.details
        ? Object.values(backendError.details)[0]
        : backendError?.message || err?.message || "Failed to update client";
  ...
  return (
    <>
      <Modal open={open} onClose={handleClose} title="Update Client Details">
